    ...
    if (segments[1] == "map") {
                    ...
                    if (htmlData !== null) {
                        ...
                        Object.keys(htmlData[0]).forEach(function(key) {
                            ...
                            if ((key !== 'type_name' && regParams[key] !== undefined) || (key === "usd1000total" || key === "tonstotal" || key === "supputotal")) {
                                html += '<th style="text-align:' + align + ';">' + translate(lang, key) + '<i class="fas fa-info-circle hideClass" id="info" style="display:' + style + ';" title="მოცემულ ველში ასახულია ის პერიოდები, როცა განხორციელდა სავაჭრო ნაკადი"></i></th>';
                            }
                        });
                        html += '</tr>';
                        html += '</thead>';
                            ...
                            Object.keys(value).forEach((nKey) => {
                                ...
                                if ((nKey !== 'type_name' && regParams[nKey] !== undefined) || nKey == 'supputotal' || (nKey === "usd1000total" || nKey === "tonstotal")) {
                                    if (nKey == 'usd1000total' || nKey == 'tonstotal' || nKey == 'supputotal') {
                                        html += '<td style="text-align:' + align + ';width:' + rowWidth + ';">' + thousands_separators(value[nKey]) + '</td>';
                                    } else {
                        ...
                        $("#table_id").DataTable({
                            ...
                            buttons: [{
                                extend: 'csv',
                                text: exportLang = (lang == 'ka') ? 'გადმოწერა' : 'Download Data',
                                charset: 'utf-8',
                                extension: '.csv',
                                fieldSeparator: ';',
                                fieldBoundary: '',
        ...
        const checkDropdownID = () => {
            checkIDs.map(items => {
                ...
                inst ance[items].on('change', function(item, state) {
        ...
        inst ance['hssec'].on('open', function(item, state) {
            hssTitle();
        });
        var obj = {};
        var arr = [];
        ...
        const getDropdownByName = () => {
            ...
            inst ance1 = tail("select#parameters", {
                locale: lang,
                multiShowCount: false,
                multiple: false,
                multiContainer: true,
            });
            drop downNames.map(items => {
                ...
                names.onchange = function(e) {
                    ...
                    if (cnt == 0) {
                        show[0].style.display = "none";
                        title3[0].style.display = "none";
                        remo veArr.map(item5 => {
                            instance[item5].config("disabled", false);
                        });
                        remo veArr2.map(item6 => {
        ...
        if (instance['country']) {
            ...
            el se
            if (instance['country'].value().includes("all")) {
        ...
        instance['country'].on('change', function(item, state) {
            ...
            if (
                ...
                Obje ct.keys(That.options.items['#']).map(
                    (elm) => {
        ...
        inst ance['grp'] && instance['grp'].on('change', function(item, state) {
            if (state == 'select') {
